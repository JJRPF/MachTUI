...
impl PluginLoader {
    ...
    pub unsafe fn load_component<P: libloading::AsFilename>(&mut self, path: P) -> Result<Box<dyn Component>, String> {
        let lib = Library::new(path).map_err(|e| e.to_string())?;
        
        type CreateComponent = unsafe extern "C" fn() -> *mut dyn Component;
        let constructor: Symbol<CreateComponent> = lib.get(b"create_component").map_err(|e| e.to_string())?;
