...
impl ShellComponent {
    ...
    pub fn render(&self, canvas: &mut Canvas, x: u16, y: u16, width: u16, height: u16) {
        ...
        for (i, line) in lines[start..].iter().enumerate() {
            let truncated = if line.len() > width as usize { &line[..width as usize] } else { line };
            canvas.draw_text(x, y + i as u16, truncated, Some(crossterm::style::Color::White));
        }
    }
}
