    ...
    loop {
        if let Some(event) = renderer.poll_event(Duration::from_millis(16))? {
            match event {
                Event::Key(KeyEvent { code, .. }) => {
                    ...
                    if code == KeyCode::Char('m') { show_modal = !show_modal; }
                }
        ...
        for x in 0..canvas.width {
            for y in 0..canvas.height {
                canvas.set_cell_z(x, y, '.', Some(Color::Rgb { r: 20, g: 20, b: 30 }), 0);
            }
        }

        // --- MAIN CONTENT (Layer 1) ---
        canvas.draw_gradient_text_z(4, 2, "JJR PRO PORTFOLIO", (0, 255, 255), (255, 0, 255), 1);
        canvas.draw_text_z(4, 4, "A display of high-end terminal engineering.", Some(Color::Grey), 1);

        let projects = ["MachTUI Engine", "AI Oracle Protocol", "Vision Graphics"];
        for (i, p) in projects.iter().enumerate() {
            let y = 7 + (i as u16 * 4);
            let b = BoxComponent { title: p.to_string(), border_color: Color::Green };
        ...
        if show_modal {
            ...
            let modal_box = BoxComponent { title: format!("{} INFORMATION", Icons::GEAR), border_color: Color::Cyan };
            ...
            canvas.draw_text_z(mx + 2, my + 2, "This is a high-z-index modal!", Some(Color::White), 11);
            canvas.draw_text_z(mx + 2, my + 4, "It overlaps all background content.", Some(Color::Grey), 11);
            canvas.draw_text_z(mx + 2, my + 8, "Press 'm' to close", Some(Color::Yellow), 11);
        }

        canvas.draw_text_z(2, canvas.height - 1, "Press 'm' for Modal | 'q' to Quit", Some(Color::DarkGrey), 50);
