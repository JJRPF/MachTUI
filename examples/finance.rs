...
async fn main() -> io::Result<()> {
    ...
    loop {
        ...
        // --- HEADER ---
        canvas.draw_gradient_text(2, 1, "MACHTUI terminal Terminal", (0, 255, 100), (0, 255, 255));
        
        // --- TICKER CARDS ---
        let cards = [("BTC/USD", Color::Yellow), ("ETH/USD", Color::Cyan), ("MCH/USD", Color::Magenta)];
        for (i, (name, color)) in cards.iter().enumerate() {
            let x = 2 + (i as u16 * 22);
            let b = BoxComponent { title: name.to_string(), border_color: *color };
            b.render(canvas, x, 3, 20, 5);
            canvas.draw_text(x + 2, 5, &format!("${:.2}", last + i as f32 * 100.0), Some(Color::White));
        }
        ...
        canvas.draw_text(2, 22, "Press 'q' to exit | Visualizing 100 data points via Braille sub-pixels", Some(Color::DarkGrey));
