...
async fn main() -> io::Result<()> {
    ...
    let mut grid = vec![vec![CellData { value: "0".to_string() }; 5]; 10];
    let mut sel_x = 0;
    let mut sel_y = 0;
    ...
    loop {
        if let Some(event) = renderer.poll_event(Duration::from_millis(16))? {
            if let Event::Key(KeyEvent { code, .. }) = event {
                match code {
                    ...
                    KeyCode::Up => if sel_y > 0 { sel_y -= 1; },
                    KeyCode::Down => if sel_y < 9 { sel_y += 1; },
                    KeyCode::Left => if sel_x > 0 { sel_x -= 1; },
                    KeyCode::Right => if sel_x < 4 { sel_x += 1; },
                    KeyCode::Char(c) => {
        ...
        for y in 0..10 {
            for x in 0..5 {
                ...
                let color = if is_sel { Color::Yellow } else { Color::DarkGrey };
                
                let cell_box = BoxComponent { title: "".into(), border_color: color };
        ...
        canvas.draw_text(2, 24, "Arrows: Navigate | Type: Edit | Q: Quit", Some(Color::Grey));
