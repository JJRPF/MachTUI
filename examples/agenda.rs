...
use machtui::core::notifications::{NotificationManager, NotificationLevel};
...
async fn main() -> io::Result<()> {
    ...
    notifications.notify("Welcome to MachAgenda!", NotificationLevel::Info, Duration::from_secs(3));
    ...
    loop {
            ...
            if let Event::Key(KeyEvent { code, .. }) = event {
                match code {
                    ...
                    KeyCode::Char('a') => {
                        ...
                        notifications.notify("Task added!", NotificationLevel::Success, Duration::from_secs(2));
                    }
                    KeyCode::Char('d') => {
                        if !tasks.is_empty() {
                            tasks.pop();
                            notifications.notify("Task removed.", NotificationLevel::Warning, Duration::from_secs(2));
                        }
                    }
                    _ => {}
                }
        ...
        for day in 1..=31 {
            ...
            let color = if day == 22 { Color::Yellow } else { Color::White };
        ...
        canvas.draw_text(2, 17, "Press 'a' to add task | 'd' to delete | 'q' to quit", Some(Color::DarkGrey));
