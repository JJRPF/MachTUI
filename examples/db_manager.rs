...
async fn main() -> io::Result<()> {
    ...
    loop {
                ...
                match code {
                    ...
                    KeyCode::Enter => {
                        if !input.content.is_empty() && !value_input.content.is_empty() {
                            db.set_kv(&input.content, &value_input.content).expect("Set failed");
                            logs.items.push(format!("SET: {} -> {}", input.content, value_input.content));
                            input.content.clear();
                    ...
                    KeyCode::Char(c) => {
                        if active_input == 0 { input.handle_char(c); } else { value_input.handle_char(c); }
                    }
                    KeyCode::Backspace => {
                        if active_input == 0 { input.handle_backspace(); } else { value_input.handle_backspace(); }
                    }
                    _ => {}
                }
            }
        ...
        // --- HEADER ---
        canvas.draw_gradient_text(2, 1, "MACHTUI DATABASE ENGINE", (255, 100, 0), (255, 200, 0));
        ...
        canvas.draw_text(2, 20, "TAB: Switch | ENTER: Save | Q: Quit", Some(Color::DarkGrey));
