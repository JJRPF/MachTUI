...
async fn main() -> io::Result<()> {
    ...
    let mut modal = Modal::new(" INSTALLATION COMPLETE ", "MachTUI has been successfully installed!");
    ...
    loop {
        if let Some(event) = renderer.poll_event(Duration::from_millis(16))? {
            if let Event::Key(KeyEvent { code, .. }) = event {
                match code {
                    ...
                    KeyCode::Enter => {
                        if step < 2 { step += 1; }
                        else if step == 2 { modal.visible = true; }
                    }
                    _ => {}
                }
            }
        ...
        // --- HEADER ---
        canvas.draw_gradient_text(2, 1, &format!("{} MACH INSTALLER PRO", Icons::ROCKET), (0, 255, 255), (255, 0, 255));
        ...
        match step {
            0 => {
                canvas.draw_text(6, 6, "Welcome to the MachTUI Setup Wizard.", Some(Color::White));
                canvas.draw_text(6, 8, "This will deploy the engine to your system.", Some(Color::Grey));
                canvas.draw_text(6, 12, "Press [ENTER] to continue...", Some(Color::Green));
            }
            1 => {
                canvas.draw_text(6, 6, "Extracting high-performance assets...", Some(Color::White));
                let pb = ProgressBar::new("Deployment", progress);
                pb.render(canvas, 6, 8, 50, 2);
                if progress >= 1.0 {
                    canvas.draw_text(6, 12, "Ready! Press [ENTER] to finalize.", Some(Color::Green));
                }
            }
            2 => {
        ...
        canvas.draw_text(2, 22, "Arrows: Nav | Enter: Action | Q: Quit", Some(Color::DarkGrey));
