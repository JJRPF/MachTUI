    ...
    loop {
        if let Some(event) = renderer.poll_event(Duration::from_millis(16))? {
            if let Event::Key(KeyEvent { code, .. }) = event {
                ...
                if code == KeyCode::Char('f') { renderer.show_fps = !renderer.show_fps; }
            }
        }
        ...
        // 2. Draw Sub-sections
        let cpu_box = BoxComponent { title: "CPU Usage".into(), border_color: Color::Green };
        ...
        let ram_box = BoxComponent { title: "Memory".into(), border_color: Color::Yellow };
        ...
        let gpu_box = BoxComponent { title: "GPU Rendering".into(), border_color: Color::Magenta };
        ...
        canvas.draw_text(6, 15, "Press 'f' to toggle FPS overlay", Some(Color::DarkGrey));
