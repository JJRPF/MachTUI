...
async fn main() -> io::Result<()> {
    ...
    loop {
        ...
        let cpu_card = BoxComponent {
            title: format!("{} CPU", Icons::CPU),
        ...
        canvas.draw_text(
            5,
            row1_y + 2,
            &format!("{:.1}% Load", next_val),
            Some(Color::White),
        );

        let ram_card = BoxComponent {
            title: format!("{} RAM", Icons::RAM),
        ...
        canvas.draw_text(
            3,
            canvas.height - 2,
            &format!("{} System Stable | Press 'q' to disconnect", Icons::SUCCESS),
            Some(Color::DarkGrey),
        );
