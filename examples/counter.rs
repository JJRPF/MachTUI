...
async fn main() -> io::Result<()> {
    ...
    let mut prog = Program::new(App { count: 0, running: true, hovered: false });
    ...
    while prog.model().running {
        ...
        let bg_color = if prog.model().hovered { Some(Color::DarkGrey) } else { None };
        let fg_color = if prog.model().hovered { Color::Yellow } else { Color::White };
        ...
        for (i, c) in count_text.chars().enumerate() {
            ...
            if let Some(bg) = bg_color {
                canvas.cells[((9 as usize) * (canvas.width as usize) + (2 + i as usize))].style.background_color = Some(bg);
            }
        }
